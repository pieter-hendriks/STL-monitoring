...
class AbsoluteValueNodeTest(UnaryOperationTest):
	...
	def testEmptySignal(self):
		...
		emptyResult: Signal = Signal('absolutevalue')
		self.quantitativeValidationTestHelper(emptySignal, emptyResult)
		emptySignal: BooleanSignal = BooleanSignal()
		emptyResult: BooleanSignal = BooleanSignal('absolutevalue')
		self.booleanValidationTestHelper(emptySignal, emptyResult)

	def testPositiveConstantSignal(self):
		# Positive signals remain unchanged
		posSignal: Signal = Signal('testsignal', [0, 1], [5, 5], [0, 0])
		expectedResult = Signal('absolutevalue', [0, 1], [5, 5], [0, 0])
		self.quantitativeValidationTestHelper(posSignal, expectedResult)
		posSignal = BooleanSignal.fromSignal(posSignal)
		expectedResult = BooleanSignal.fromSignal(expectedResult)
		self.booleanValidationTestHelper(posSignal, expectedResult)

	def testPositiveVariableSignal(self):
		# Positive signals remain unchanged
		posSignal: Signal = Signal('testsignal', [0, 1, 2], [3, 5, 7], [2, 2, 0])
		expectedResult = Signal('absolutevalue', [0, 1, 2], [3, 5, 7], [2, 2, 0])
		self.quantitativeValidationTestHelper(posSignal, expectedResult)
		posSignal = BooleanSignal.fromSignal(posSignal)
		expectedResult = BooleanSignal.fromSignal(expectedResult)
		self.booleanValidationTestHelper(posSignal, expectedResult)
	...
	def testNegativeConstantSignal(self):
		...
		resSignal: Signal = Signal('absolutevalue', [0, 1, 2], [1, 1, 1], [0, 0, 0])
