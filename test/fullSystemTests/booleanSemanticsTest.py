...
class BooleanSemanticsTest(unittest.TestCase):
	...
	def testValidatedSignal(self):
		...
		self.assertEqual(expectedOutput.booleanValidatedSignal, result, f"Boolean validated signal result mismatch for input {signalFiles.booleanValidatedSignal} (validated signal)")

	def testInvalidatedSignal(self):
		signal = SignalList.fromCSV(signalFiles.booleanInvalidatedSignal)
		result = self.stlTree.validate(signal, 'boolean', plot=False)
		self.assertEqual(expectedOutput.booleanInvalidatedSignal, result, f"Boolean invalidated signal result mismatch for input {signalFiles.booleanInvalidatedSignal} (invalidated signal)")
...
class BooleanCartpoleTest(unittest.TestCase):
	...
	def testValidatedSignal(self):
		signal = SignalList.fromCSV(signalFiles.cartpoleBooleanSignal)
		result = self.stlTree.validate(signal, 'boolean', plot=False)
		self.assertEqual(expectedOutput.booleanCartpoleSignal, result, "Boolean cartpole result mismatch.")
