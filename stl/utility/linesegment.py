...
class LineSegment:
	...
	def intersects(self, other: 'LineSegment') -> bool:
		...
		# ACD and BCD must have opposite orientation if the line segments intersect - ACD and BCD cannot both be counterclockwise (unless there is no intersection)
		# Analogous for ABC and ABD
		return (
		    Point.inCounterClockWiseOrder(self.A, other.A, other.B) !=
		    Point.inCounterClockWiseOrder(self.B, other.A, other.B)
		    and Point.inCounterClockWiseOrder(self.A, self.B, other.A) !=
		    Point.inCounterClockWiseOrder(self.A, self.B, other.B)
		)
	...
	def computeIntersectionPoint(self, other: 'LineSegment') -> Point:
		...
		assert self.intersects(
		    other
		), "Cannot find an intersection point for two line segments that do not intersect."
		...
		), f"Failed to find an intersection! Computed y-values do not match."
	...
	def computeIntersectionPoints(
	    cls, lhsSegments: Union[List['LineSegment'], 'LineSegment'],
	    rhsSegments: Union[List['LineSegment'], 'LineSegment']
	) -> List[Point]:
		...
		while True:
			...
			if lhsIndex >= len(lhsSegments) - 1:
			    ...
			    rhsIndex >= len(rhsSegments) - 1
			    or lhsSegments[lhsIndex + 1].B.x <= rhsSegments[rhsIndex + 1].B.x
			):
				lhsIndex += 1
			else:
				rhsIndex += 1
