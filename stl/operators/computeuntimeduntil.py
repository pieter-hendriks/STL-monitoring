...
def computeUntimedUntil(lhsSignal: Signal, rhsSignal: Signal) -> Signal:
	...
	allTimes: List[float] = getSortedMergedListNoDuplicates(
	    lhsSignal.getTimes(), rhsSignal.getTimes()
	)
	...
	while currentIndex >= 0:
		currentInterval: Interval = Interval(
		    lhsSignal.getTime(currentIndex), lhsSignal.getTime(currentIndex + 1)
		)
		...
		if lhsSignal.getDerivative(currentIndex) <= 0:
			constLhsUpper: Signal = signalType.createConstant(
			    'yconst', lhsSignal.getValue(currentIndex + 1),
			    [currentInterval.getLower(), currentInterval.getUpper()]
			)  # LHS(t) == OK
			previousAndConstLhsUpper: Signal = computeAnd(
			    constLhsUpper, previousValue
			)  # MIN(lhs(t), z(t)) == OK
			lhsAndRhs: Signal = computeAnd(
			    currentRhsInterval, currentLhsInterval
			)  # MIN(rhs(tau), lhs(tau)) == OK
			eventuallyLhsAndRhs: Signal = computeUntimedEventually(
			    lhsAndRhs
			)  # SUP(MIN(rhs(tau), lhs(tau))) (==z_t(s)) == OK
			outputSegment: Signal = computeOr(
			    eventuallyLhsAndRhs, previousAndConstLhsUpper
			)  # MAX(zt(s), min(LHS,OLD)) == OK
		else:
			leftConstSignal: Signal = signalType.createConstant(
			    "leftconst", lhsSignal.getValue(currentIndex),
			    [currentInterval.getLower(), currentInterval.getUpper()]
			)  # LHS(s) == OK
			eventuallyRhs: Signal = computeUntimedEventually(currentRhsInterval)  # SUP(rhs) == OK
			lhsAndEventuallyRhs: Signal = computeAnd(
			    eventuallyRhs, leftConstSignal
			)  # MIN(SUP(RHS), LHS(s)) == OK
			...
			outputSegment: Signal = computeOr(
			    lhsAndEventuallyRhs, lhsAndPrevious
			)  # MAX(zt(s), min(LHS,OLD)) == OK
