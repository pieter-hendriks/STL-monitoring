...
class Signal:
	...
	def __init__(
		...
		if times is not None:
			...
			), "debug assert: times mustn't be equal"
		...
		self.checkpoints: SortedList[SignalValue] = SortedList(
		    [SignalValue(x, y, d) for x, y, d in zip(safeTime, safeValue, safeDeriv)],
	...
	def fromBooleanSignal(cls, s: 'Signal') -> 'Signal':
		...
		times, values, derivatives = zip(
		    *[(cp.getTime(), cp.getValue(), cp.getDerivative()) for cp in s.checkpoints]
		)
	...
	@classmethod
	def computeCheckpointsForComparableSignal(cls, lhsSignal: 'Signal',
	                                          rhsSignal: 'Signal') -> Tuple['Signal', 'Signal']:
	...
	def computeInterval(self, interval: Interval, half_open: bool = False) -> 'Signal':
		...
		output = self.fromCheckpoints(
		    constructedSignalName, self.checkpoints[lowerBoundIndex:upperBoundIndex + 1]
		)
		...
		) and interval.getUpper() < self.getTime(-1):
	...
	def computeLines(self) -> List[LineSegment]:
		...
		for i in range(self.getCheckpointCount() - 1):
			...
			ret.append(
			    LineSegment(Point(cpA.getTime(), cpA.getValue()), Point(cpB.getTime(), cpB.getValue()))
			)
		return ret
	...
	def merge(self, other: 'Signal') -> None:
		...
		assert all(
		    [
		        cp.getTime() not in self.getTimes()
		        or self.getCheckpoint(self.computeIndexForTime(cp.getTime())) == cp
	...
	def recomputeDerivatives(self):
		...
		if self.checkpoints:  # no-op if empty list
			for i in range(len(self.checkpoints) - 1):
				...
				if self.checkpoints[i + 1].getValue() - self.checkpoints[i].getValue(
				) != 0 and self.checkpoints[i].getDerivative() == 0:
					assert False, "wtf"
