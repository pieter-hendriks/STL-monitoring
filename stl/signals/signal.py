...
class Signal:
	...
	def __init__(
		...
		safeTime = times if times is not None else list(range(len(safeValue)))
		...
		# Initialize
		self.checkpoints: Iterable[SignalValue] = SortedList(
		...
		self.times: Iterable[float] = SortedList(safeTime)
	...
	def computeInterpolatedDerivative(self, t: float) -> float:
		...
		raise RuntimeError("Unexpected case; unable to find derivative for specific time.")
	...
	def lenientEquals(self, other: 'Signal') -> bool:
		"""Checks if self == other, allowing either one to have extra checkpoints
		(assuming their value is close to what would be expected on the current derivative)."""
		# Name mismatch -> False, different intervals over which they're defined -> False
		if self.name != other.name:
			return False
		if self.getDefinedTimeInterval() != other.getDefinedTimeInterval():
			return False
